...
if __name__ == "__main__":
    ...
    if not os.path.exists(excel_path):
        print("Sample data file not found. Please provide an Excel file with drilling data.")
        print("Expected columns: formation type, WOB, RPM, flow rate, pump pressure, density, "
              "funnel viscosity, theta3, theta300, theta600, drilling time")
    else:
        ...
        print(f"\nFuzzy match top-{topk} plan IDs: {result_ids}")
        if result_ids:
            print("Matched plan data:")
            for sid, data in zip(result_ids, result_data):
                print(f"  Plan {sid}: {data}")
        else:
            print("No similar plans found")
