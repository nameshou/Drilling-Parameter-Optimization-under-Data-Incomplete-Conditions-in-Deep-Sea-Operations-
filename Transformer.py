...
def load_and_preprocess_data(file_path, target_col='钻时', seq_length=10, augment_factor=1):
...
def train_main():
    ...
    file_path = "E:/tool/python project/test/钻速机器学习.xlsm"
    seq_length = 20
    batch_size = 32
    epochs = 500
    lr = 0.001
...
def predict_from_list(data_list, model_path='transformer_model.pth', scaler_path='scaler_X.pkl',
    ...
    if len(X_scaled) <= seq_length:
        raise ValueError(f"数据行数必须大于 {seq_length}")
...
if __name__ == "__main__":
    ...
    if mode == 'train':
        ...
        seq_length = 20
        if len(a) <= seq_length:
            print(f"错误：数据行数 {len(a)} 不足 {seq_length+1}")
        else:
