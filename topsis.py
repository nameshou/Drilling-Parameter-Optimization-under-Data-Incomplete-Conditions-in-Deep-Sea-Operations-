...
def forward_transform(matrix, types, bounds=None):
    ...
    for j in range(n):
        ...
        if t == 'max':
            ...
            raise ValueError("Indicator type must be one of 'max', 'min', 'mid', 'range'")
    return result
...
if __name__ == "__main__":
    ...
    indicator_names = ['WOB (kN)', 'RPM', 'Flow rate (r/min)',
                   'Pump pressure (MPa)', 'Density (g/cm^3)',
                   'Funnel viscosity (s)', 'theta3', 'theta300', 'theta600',
                   'Drilling time']
    ...
    print("Forward-transformed decision matrix:")
    ...
    print("\nAHP subjective weights:", np.round(subj_weights, 4))
    print("Consistency ratio CR =", round(cr, 4))
    print("Matrix shape:", original_matrix.shape)
    print("types length:", len(indicator_types))
    print("bounds length:", len(bounds))

    obj_weights = entropy_weight(forward_matrix)
    print("Entropy method objective weights:", np.round(obj_weights, 4))

    fused_weights = fuse_weights_multiplicative(subj_weights, obj_weights)
    print("Multiplicative fusion weights:", np.round(fused_weights, 4))

    impacts = ['+'] * len(indicator_types)
    scores, rank = topsis(forward_matrix, fused_weights, impacts)
    print("\nTOPSIS evaluation results:")
    for i in range(len(scores)):
        print(f"Plan {i+1}: closeness = {scores[i]:.4f}, rank = {np.where(rank == i)[0][0]+1}")
